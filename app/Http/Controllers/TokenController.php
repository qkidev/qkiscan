...
class TokenController extends Controller
{
    ...
    public function index($address)
    {
        ...
        $data['transactions_num'] = TokenTx::where('tx_status', 1)->count();
        $start = time()-24*60*60-28800;
        $end = time()-28800;
        $data['hour_24_num'] = TokenTx::where('tx_status', 1)->whereTime('created_at', '<', $end)->whereTime('created_at', '>', $start)->count();
        $data['hour_24_amount'] = TokenTx::where('tx_status', 1)->whereTime('created_at', '<', $end)->whereTime('created_at', '>', $start)->sum('amount');
        $data['address_num'] = Balances::where('cct', '>', 0)->count();
        // token top 100
